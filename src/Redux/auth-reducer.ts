...
const slice = createSlice({
    ...
    reducers: {
        setAuthUserDataAC(state, action: PayloadAction<{
            userId: number | null, email: string | null, login: string
                | null, isAuth: boolean
        }>) {
...
export const logoutTC = () => async (dispatch: AppDispatch) => {
    ...
    if (res.data.resultCode === 0) {
        dispatch(setAuthUserDataAC({userId: null, email: null, login: null, isAuth: false}))
